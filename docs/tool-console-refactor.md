...
关闭。实际完成独立管理员/普通账号登录、双前端入口、目录全员可见、私有脚本创建、单脚本控制台切换、草稿与仅运行执行、表格/日志、管理账号及
SQLite 生产模拟环境视图。
使用 12 个参数、87 行代码、45 行表格验证参数滚动提示、固定目标/运行入口、表格分页/排序/空筛选；390px
窄屏表单按顺序排列、工作台参数抽屉可用，没有页面横向溢出。编辑器和参数区约 62/38，结果区永久可见，用户菜单不遮挡结果操作。
行号与代码对齐；保存后补全连接恢复的回归已通过。浏览器 local 管理员演示不替代上述多身份后端测试。
...
- 未登录直达页面保留安全的相对返回地址；`/admin` 与 `/workspace` 使用不同 HTML/React 入口。可信身份头模式需显式配置，不会降级到本地密码或
  Mock。
- `/workspace` 与脚本页共用资源栏和控制台外壳，不再维护独立首页或顶部多脚本 Tab；同一时刻只挂载一个控制台，根路径保持空控制台，不持久化运行值和结果。
- 目录在点击前批量展示当前账号能力，选中文件夹即成为新建位置；工具栏按主要操作、回溯、工具设置、开发辅助分组；结果永久可见；示例库增加指标、趋势、明细、JSON、文本与空结果。
- 生产或操作类脚本使用页面内二次确认，确认前标记“尚未发起执行”，取消与校验失败都会在结果区留下明确状态。
