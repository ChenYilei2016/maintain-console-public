...
1. 登录后进入 `/workspace`，从资源树直接打开脚本。选中文件夹再点 **新建**，脚本会创建到该位置；末级文件夹仍可创建脚本。
   所有启用账号都能创建自己的脚本；新脚本默认只把创建者写入读、编、执、管四项 JSON 权限。
2. 右侧 **参数配置 → 添加参数**：设置技术名称、中文显示名、用途/示例、类型、默认值、分组及校验范围。
3. 左侧使用 `def count = $${count}`。类型化参数是独立的数据表达式，不要额外加引号，也不要放入注释、字符串或标识符内部。
4. **用途与风险** 填写说明、输入示例和操作类型；**运行填值** 填写本次值，点击 **调试当前内容**
   。生产环境或操作类脚本会先展示页面内二次确认，明确标记“尚未发起执行”；确认后才发送请求。草稿调试同时要求编辑与执行权限，不自动保存。
...
工作台采用左侧编辑器、右侧参数、底部结果的布局。右侧切换 **运行填值 / 参数配置**，执行按钮始终位于侧栏底部；结果永久展开，可放大及还原。
工作台同一时刻只挂载一个脚本控制台，资源树切换使用应用内导航，不保留隐藏页面或顶部脚本 Tab；切换前会保护未保存草稿和未结束请求。
...
资源树在点击前展示当前账号的可调试、可运行、可编辑、只读、可授权或仅目录能力；没有任何脚本能力的节点不可点击，只有授权管理能力时可直接进入授权面板。
